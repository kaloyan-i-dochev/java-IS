...
public class SimpleHeuristicPuzzleSolver extends PuzzleSolver { // AKA Best First Search
    public SimpleHeuristicPuzzleSolver(PuzzleState initialState) {
        super(initialState);
    }
    ...
    public PuzzleSolution solve() {
        ...
        PriorityQueue<Node> frontier = new PriorityQueue<>(
                Comparator.comparingInt(node -> node.heuristicCost));
        Node root = new Node(initialState, null, null, manhattanDistance(initialState));
        frontier.add(root);
        ...
        while (!frontier.isEmpty()) {
            ...
            for (Direction direction : Direction.values()) {
                if (currentNode.state.canMove(direction)) {
                    ...
                    Node childNode = new Node(nextState, currentNode, direction, heuristicCost);
    ...
    private static class Node {
        ...
        Node(PuzzleState state, Node parent, Direction action, int heuristicCost) {
            this.state = state;
            this.parent = parent;
            this.action = action;
